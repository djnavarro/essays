...
summary: "The story of my preparations for my first rape trauma counselling session, told mostly in fits and starts. There are a lot of unpleasant memories jumbled in here, and they don't necessarily map onto the people or events one might think they do."
---
...
```
```{r, include = FALSE}
#pagecolour <- "white" #unname(slumdown::slum_palette("dark")["pagecolour"])
#slumdown::slum_plot_background(palette = "dark", name = "pagecolour")
```
```{r include=FALSE}
teacup <- function(...) {
  # do nothing
}
# teacup <- function(seed, iterations = 1, burnin = 0, scale = .01) {
#   ribbon <- jasmines::time_tempest(seed,     
...
<p>*You know he has a thing for the trans girls, right?*</p>
<p align="right">*(Yeah, I know... but it's not like we've got options, right? Now be a good girl and pull down your skirt)*</p>

```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=1, fig.asp=1, fig.align='right'}
set.seed(192)
teacup(jasmines::seed_bubbles(1, 100), scale = .015) 
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=1, fig.asp=1, fig.align='center'}
set.seed(1)
teacup(jasmines::seed_sticks(2, 100)) 
```

*So do you have breasts now?*<br>
*Are they real?*<br>
*I want to see -- show me!*<br> 
*Come on, show me, no need to be shy!*

<p align = "right">*(God I hate it when you drink)*</p>

```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=1, fig.asp=1, fig.align='center'}
set.seed(10)
teacup(jasmines::seed_bubbles(50, 10), iterations = 3) 
```
...
<p align="right">*(Well what the fuck were you expecting, girl?)*</p>

```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=1, fig.asp=1, fig.align='center'}
set.seed(1)
teacup(jasmines::seed_heart(100), iterations = 3, burnin = 1, scale = .05) 
```


*Are you going to have the surgery?* <br>
*Or did you already?*<br>
*Are you getting a divorce?* <br>
*Do you want a husband now?* <br>

<p align="right">*(Uh yeah ... I don't really know you and I'm only here because I can't work out how to use the photocopier and I guess I don't know what any of these questions have to do with that?)*</p>



```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=1, fig.asp=1, fig.align='center'}
set.seed(1)
teacup(jasmines::seed_rows(4, 70), scale = .01, iterations = 4, burnin = 3) 
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=1, fig.asp=1, fig.align='center'}
set.seed(1)
teacup(jasmines::seed_text("anthea"), iterations = 25, burnin = 20, scale = .04) 
```
<br><br>
<p>Is that all?</p>
<p align="right">*(What the fuck? What is wrong with you? I didn't want you to do any of that in the first place. I didn't want it the last two times either. I want you out of my apartment and my life. If I let you fuck me will you really go this time? Please just leave. Please)*</p>
<p align="right">...</p>
<p align="right">*(Oh fucking brilliant. Well, you know what happens now, don't you honey?)*</p>


```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=1, fig.asp=1, fig.align='right'}
set.seed(1)
teacup(jasmines::seed_bubbles(5, 100)) 
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=1, fig.asp=1, fig.align='left'}
set.seed(1)
teacup(jasmines::seed_rows(10, 10), scale = .015) 
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=1, fig.asp=1, fig.align='center'}
set.seed(1)
teacup(jasmines::seed_bubbles(20, 5), iterations = 6) 
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=1, fig.asp=1, fig.align='right'}
set.seed(1)
teacup(jasmines::seed_text("|"), iterations = 50, burnin = 5) 
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=1, fig.asp=1, fig.align='right'}
set.seed(1)
teacup(jasmines::seed_text("oh"), iterations = 50, burnin = 5) 
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=1, fig.asp=1, fig.align='center'}
set.seed(19)
teacup(jasmines::seed_bubbles(1, 1000), scale = .005) 
```
...
<p align="right">*(Well you shouldn't have worn those jeans, sweetie)*</p>
...
<p align="right">*(See? You must have wanted it. And you have the gall to call yourself a rape survivor? It's only rape if it comes from the French region of man-with-a-knife-at-your-throat. Otherwise it's just sparkling sexual regret)*</p>
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=1, fig.asp=1, fig.align='center'}
set.seed(1)
teacup(jasmines::seed_rows(1, 3) %>% jasmines::time_meander() %>% select(-time)) 
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=1, fig.asp=1, fig.align='center'}
set.seed(1)
teacup(jasmines::seed_sticks(1, 100)) 
```
...
set.seed(2)
n <- 20
teacup(jasmines::seed_rows(1, 50), iterations = n+1, burnin = n, scale = .01) 
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=1, fig.asp=1, fig.align='center'}
set.seed(1)
teacup(jasmines::seed_bubbles(1, 1) %>% jasmines::time_meander() %>% select(-time)) 
```
...
<p align="right">*(You're not though, are you?)*</p>
